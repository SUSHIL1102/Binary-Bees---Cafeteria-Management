...
const w3Auth = require("./auth/w3Auth");
...
// W3 SSO routes
app.get("/auth/login", w3Auth.login);
app.get("/auth/callback", w3Auth.callback);

// Legacy login (temporary, documented)
app.post("/auth/legacy-login", (req, res) => {
  res.status(410).json({
    message: "Deprecated. Use /auth/login with W3 SSO"
  });
});
